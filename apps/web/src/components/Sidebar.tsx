...
const nav = [
  { href: "/dashboard",       label: "My Groups", icon: LayoutDashboard },
  { href: "/dashboard/guide", label: "Guide",     icon: BookMarked },
];

export function Sidebar() {
  const path = usePathname();
  const isGroup = path.startsWith("/dashboard/groups/");
  if (isGroup) return null; // per-group pages have their own nav
  ...
  return (
    <>
      {/* Desktop sidebar */}
      <aside className="hidden md:flex w-48 min-h-screen flex-col bg-[#14141f] border-r border-white/5 shrink-0">
