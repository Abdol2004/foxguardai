...
export default async function AnalyticsPage({
  ...
  const data = chatId ? await getLast7Days(chatId) : [];
  const maxMsg = Math.max(...data.map((d) => d.messages), 1);

  return (
    <div className="p-4 md:p-6 animate-in">
      <h1 className="text-base font-semibold text-white mb-1">Analytics</h1>
      <p className="text-xs text-[#64748b] mb-5">7-day activity</p>

      {!chatId && (
        <div className="bg-[#14141f] border border-white/5 rounded-xl p-6 text-center text-sm text-[#64748b]">
          Add <code className="text-[#f97316]">?chatId=-100xxx</code> to the URL
        </div>
      )}
            ...
            <div className="flex items-end gap-1.5 h-24">
              {data.map((d) => (
                <div key={d.date} className="flex-1 flex flex-col items-center gap-1">
                  <div
                    className="w-full bg-[#f97316] rounded-sm min-h-[2px] transition-all"
